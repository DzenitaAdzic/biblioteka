...
public class BibliotekarStudenti extends JFrame {
    ...
    private String tabelaKolone[] = {"Sifra", "Broj Indeksa", "Ime", "Prezime", "Negativni bodovi"};
	...
	public BibliotekarStudenti(Korisnik k) {
		this.korisnik = k;
		setTitle("Studenti");
		...
		lblBrojIndeksa.setBounds(18, 101, 127, 16);
		getContentPane().add(lblBrojIndeksa);
		...
		txtBrojIndeksa.setBounds(157, 96, 344, 26);
		getContentPane().add(txtBrojIndeksa);
		...
		cmbFilter.setModel(new DefaultComboBoxModel(new String[] {"sifra", "broj indeksa", "ime", "prezime"}));
		...
		btnPosudba.setBounds(18, 440, 110, 29);
		getContentPane().add(btnPosudba);
		btnPosudba.setVisible(false);
		...
		btnDodati.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(txtIme.getText().length()==0){
					JOptionPane.showMessageDialog(null, "Polje Ime je obavezno.", "Greska", JOptionPane.ERROR_MESSAGE);
					txtIme.requestFocus();
					return;
				}
				...
				if(txtSifra.getText().length()>10){
					JOptionPane.showMessageDialog(null, "Maksimalna duz�ina polja sifra je 10 karaktera.", "Greška", JOptionPane.ERROR_MESSAGE);
					txtSifra.requestFocus();
					return;
				}		
				if(GlobalHelper.isNumber(txtNegativniBodovi.getText())==false){
					JOptionPane.showMessageDialog(null, "Polje Negativni bodovi mora biti broj.", "Greska", JOptionPane.ERROR_MESSAGE);
			...
			if (table.getSelectedRowCount()==0) {
	            ...
	            switch (trenutniMod) {
	                ...
	                case IZMJENA:
		            	...
		            	txtBrojIndeksa.setText(odabraniObjekat.getBrojIndeksa());
		            	txtSifra.requestFocus();
	                    break;
	            }
	        }			
				...
				if (table.getSelectedRowCount()==0) {
		            ...
		            odabraniObjekat=service.find(sifra);
		             int odgovor = JOptionPane.showConfirmDialog(null,
		                "Jeste li sigurni da zelite obrisati unos " + odabraniObjekat.toString() + "?",
		                "Brisanje",
	...
	private void osvjeziTabelu(){
        ...
        for(Student student : lista){
            Object[] data={student.getSifra(), student.getBrojIndeksa(), student.getIme(), student.getPrezime(), student.getNegativniBodovi()};            
